...
sprintf(str, "%d", a);
	...
	strcpy(query, "DELETE FROM save WHERE id=");
	strcat(query, str);
...
//	printf("<form action=\"%s\" method=\"POST\">",b);
	...
	if (strcmp(b, "scene6.cgi")==0)
		printf("<script type=\"text/javascript\">post(\'scene7.cgi\', {'scene7.cgi': 11});</script>");
	else
		printf("<script type=\"text/javascript\">post(\'%s\', {'%s': 11});</script>",b,b);
//	printf("</form>");
	printf("	</body></html>");
return 0;
}
