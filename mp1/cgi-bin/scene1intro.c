...
//Correction of + to space
	...
	strcpy(query, "DELETE FROM savetemp");
	mysql_real_connect(con, "localhost", "root", "root", "mp", 0, NULL, 0);
	mysql_query(con, query);
	strcpy(query, "INSERT INTO savetemp VALUES(11, CONCAT(curdate(),' ',curtime()), 0, 10, 0, 0, 0, 'scene1.cgi', 0)");
	mysql_query(con, query);
	...
	printf("	</head>");
	printf("	<body onload=\"post(\'scene1.cgi\', {'scene1.cgi': 11});\">");
}
